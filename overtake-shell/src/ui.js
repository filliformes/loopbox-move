...
punchVals[11] = [0.4, 0.7, 0.4, 1.0];   /* Shimmer: regen, pitch up, darker tone */
...
const MENU_NAMES = ['Input FX', 'Global FX', 'Perform', 'Settings'];
...
const PREAMP_NAMES = ['Clean','Cass1','Cass2','VHS1','VHS2','Reel15','Reel7','Reel3','4trk','Porta','Dub','Warp'];
const MENU_DEFS = [
    [ /* Track 1 — Input FX */
    ...
    [ /* Track 4 — Settings */
      ...
      { k:'stability', lo:0, hi:1, lbl:'Stab' },      { k:'globalWowFlut', lo:0, hi:1, lbl:'gWF' },
    ],
];
...
const PAGE0 = [   /* Loop page 1 (Up arrow) — knob 8 = Send A */
    { k: 'v_pitch', lo: -2, hi: 2, lbl: 'Pit', spd: true, step: 0.1 / 12 }, { k: 'v_filter', lo: 0, hi: 1, lbl: 'Fil' },
...
const PAGE1 = [   /* Loop page 2 (Down arrow) — knobs 6/7 = Scatter/Seed, knob 8 = Send B */
    { k: 'clock', lo: 0, hi: 1, lbl: 'Clk', clk: true },  { k: 'v_tilt', lo: -1, hi: 1, lbl: 'Tlt' },
...
const PAGE2 = [   /* Loop page 3 / Tone (Right arrow) — Studer EQ + DJ reso + amp envelope */
    ...
    { k: 'v_djReso', lo: 0, hi: 1, lbl: 'Reso' },     { k: 'v_filter', lo: 0, hi: 1, lbl: 'Fil' },
    { k: 'v_atk', lo: 0, hi: 1, lbl: 'Atk' },         { k: 'v_rel', lo: 0, hi: 1, lbl: 'Dec' },
];
const PAGES = [PAGE0, PAGE1, PAGE2];
function page() { return loopPage; }
function setPage(p) { p = p < 0 ? 0 : (p > 2 ? 2 : p); if (p !== loopPage) { loopPage = p; needReload = true; dirty = true; paintNav(); } }
...
function paintNav() {
    setButtonLED(MoveUp,    WhiteLedDim, true);
    setButtonLED(MoveDown,  loopPage === 1 ? WhiteLedBright : WhiteLedDim, true);
    setButtonLED(MoveLeft,  loopPage === 0 ? WhiteLedBright : WhiteLedDim, true);
    setButtonLED(MoveRight, loopPage === 2 ? WhiteLedBright : WhiteLedDim, true);
...
function menuKnob(k, delta) {
    const d = MENU_DEFS[menu][k];
    if (d.trig) { if (delta !== 0) { sp(d.k, '1'); lastKnob = k; lastKnobLbl = d.lbl; lastKnobVal = 'fire'; } return; }
    if (d.opts) {
...
function handleKnobTouch(d1) {
    ...
    if (menu >= 0 && MENU_DEFS[menu]) {
        const d = MENU_DEFS[menu][k]; lastKnobLbl = d.lbl;
        lastKnobVal = d.trig ? '(fire)' : (d.opts ? d.opts[Math.round(menuVals[k])] : (d.int ? String(Math.round(menuVals[k])) : Number(menuVals[k]).toFixed(2)));
...
globalThis.onResume = function () { paintAll(true); paintAll(false); paintAll(false); };
...
globalThis.onMidiMessageInternal = function (data) {
  try {
    ...
    if (status === 0xb0) {                          /* CC: knobs + buttons */
        ...
        if (d1 === MoveRight && d2 > 0) { setPage(2); return; }              /* jump to Tone page */
        if (d1 === MoveLeft  && d2 > 0) { setPage(0); return; }              /* jump to Main page */
    ...
    if (status === 0x90 && d2 > 0) {                /* note-on */
        ...
        if (d1 in STEP_TO_TRACK) { if (menu >= 0) { menu = -1; paintTrackLEDs(); } selectTrack(STEP_TO_TRACK[d1]); return; }
